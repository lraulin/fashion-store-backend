...
describe("orders", function () {
  ...
  it("should list ALL orders on /orders GET", async () => {
    try {
      ...
      if (order.ship_date === null)
        ...
        res.body[0].ship_date.should.strictEqual(order.ship_date.toISOString());
  ...
  it("should list a SINGLE order on /order/<id> GET", async () => {
    try {
      const order = await new Order(order1).save();
  ...
  it("should add a SINGLE order on /orders POST when a single order is provided", async () => {
  ...
  it("should add MULTIPLE orders on /orders POST when an array is provided", async () => {
  ...
  it("should update a SINGLE order on /order/<id> PUT", async () => {
    const requestBody = {
      ship_date: new Date(),
      status: "shipped",
    };
  ...
  it("should delete a SINGLE order on /order/<id> DELETE", async () => {
    try {
      const order = await new Order(order1).save();
  ...
  it("should return error on /order/<id> DELETE with invalid ID", async () => {
